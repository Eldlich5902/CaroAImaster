...
public class SettingDao {
	public static Setting LoadSettingInfo() {
		...
		if (!file.exists()) {
			try {
				...
				return new Setting(Value.BACKGROUND_COLOR, Value.CELL_COLOR, Value.USER_TEXT_COLOR, Value.AI_TEXT_COLOR, Value.DEFAULT_MODE, Value.DEFAULT_PVPMODE);
			}
		}
		try {
			// Load data unicode
			...
			int pvp = Integer.parseInt(reader.readLine());
			Setting setting = new Setting(backgroundColor, cellColor, xColor, oColor, mode, pvp);
			reader.close();
			return setting;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void SaveSettingInfo(Setting setting) {
		...
		try {
			...
			writer.write(String.valueOf(setting.getPvP()) + "\n");
