...
import com.caro.bean.RoundedBorder;
import com.caro.bean.Setting;
import com.caro.bo.CaroAI;
import com.caro.dao.SettingDao;
import com.caro.values.Value;
...
public class App extends JFrame implements MouseListener {
	...
	private final ButtonGroup GroupPvP = new ButtonGroup();
	...
	public void newGame() {
		...
		caro = new CaroAI(setting.getMode(), setting.getPvP());
	...
	public App() {
		...
		setting = SettingDao.LoadSettingInfo();
		caro = new CaroAI(setting.getMode(), setting.getPvP()); // khởi tạo CaroAI
		...
		if (setting.getMode() == 1 && setting.getPvP() == 1)
		...
		JLabel lblPlayerGofirst = new JLabel("Player or Bot go first:");
		lblPlayerGofirst.setHorizontalAlignment(SwingConstants.LEFT);
		lblPlayerGofirst.setForeground(new Color(0, 0, 139));
		lblPlayerGofirst.setFont(new Font("Comic Sans MS", Font.PLAIN, 16));
		lblPlayerGofirst.setBounds(10, 150, 254, 20);
		view.add(lblPlayerGofirst);
		...
		lblMode.setBounds(10, 220, 254, 20);
		view.add(lblMode);
		...
		btnNewGame.setBounds(30, 460, 89, 37);
		...
		btnExitGame.setBounds(156, 460, 89, 37);
		view.add(btnExitGame);
		// Chọn xem người hay máy đi trước
		...
		rdbtnUserPlaysFirst.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (setting.getMode() == 1) {
					int result = JOptionPane.showConfirmDialog(null, "Xác nhận đổi chế độ chơi? User plays first",
		...
		rdbtnUserPlaysFirst.setBounds(26, 175, 232, 23);
		...
		rdbtnAiPlaysFirst.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (setting.getMode() == 0) {
					int result = JOptionPane.showConfirmDialog(null, "Xác nhận đổi chế độ chơi? AI plays first",
							"Xác nhận", JOptionPane.YES_NO_OPTION);
					if (result == JOptionPane.YES_OPTION) {
		...
		rdbtnAiPlaysFirst.setBounds(26, 200, 232, 23);
		view.add(rdbtnAiPlaysFirst);

		// Chuyển chế độ chơi PvP và PvE
		JRadioButton rdbtnPvP = new JRadioButton("Player Vs Player");
		JRadioButton rdbtnPvE = new JRadioButton("Player Vs Bot");
		if (setting.getPvP() == 0)
			rdbtnPvP.setSelected(true);
		else
			rdbtnPvE.setSelected(true);

		rdbtnPvP.setForeground(new Color(107, 142, 35));
		rdbtnPvP.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (setting.getPvP() == 1) {
					int result = JOptionPane.showConfirmDialog(null, "Xác nhận đổi chế độ chơi? Player Vs Player",
							"Xác nhận", JOptionPane.YES_NO_OPTION);
					if (result == JOptionPane.YES_OPTION) {
						setting.setPvP(0); // cập nhật chế độ
						newGame(); // clear màn chơi cũ
					} else {
						rdbtnPvP.setSelected(false);
						rdbtnPvE.setSelected(true);
					}
				}
			}
		});
		rdbtnPvP.setFont(new Font("Comic Sans MS", Font.PLAIN, 14));
		GroupPvP.add(rdbtnPvP);
		rdbtnPvP.setOpaque(false);
		rdbtnPvP.setBounds(26, 240, 232, 23);
		view.add(rdbtnPvP);

		rdbtnPvE.setForeground(new Color(107, 142, 35));
		rdbtnPvE.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (setting.getPvP() == 0) {
					int result = JOptionPane.showConfirmDialog(null, "Xác nhận đổi chế độ chơi? Player Vs Bot",
							"Xác nhận", JOptionPane.YES_NO_OPTION);
					if (result == JOptionPane.YES_OPTION) {
						setting.setPvP(1); // cập nhật chế độ
						newGame(); // clear màn chơi cũ
					} else {
						rdbtnPvP.setSelected(true);
						rdbtnPvE.setSelected(false);
					}
				}
			}
		});
		rdbtnPvE.setFont(new Font("Comic Sans MS", Font.PLAIN, 14));
		GroupPvP.add(rdbtnPvE);
		rdbtnPvE.setOpaque(false);
		rdbtnPvE.setBounds(26, 270, 232, 23);
		view.add(rdbtnPvE);
		...
		separator_2.setBounds(10, 300, 254, 2);
		view.add(separator_2);

		JSeparator separator = new JSeparator();
		separator.setBackground(Color.GRAY);
		separator.setForeground(Color.DARK_GRAY);
		separator.setBounds(10, 440, 254, 2);
		view.add(separator);
		//
		...
		btnInfo.setBounds(156, 520, 89, 37);
		view.add(btnInfo);
		...
		btnIntroduce.setBounds(30, 520, 89, 37);
		...
		btnBackgroundColor.setBounds(30, 390, 89, 28);
		...
		btnCellColor.setBounds(156, 390, 89, 28);
			...
			if (caro.isClickable(x, y)) {// nếu ô này chưa được đánh
				...
				if (setting.getPvP() == 1) {
					...
					if (checkResult(Value.AI_VALUE))
						return;
				} else if (setting.getPvP() == 0) {
					if (caro.nuocdi % 2 != 0) {
						caro.update(x, y, Value.USER_VALUE);
						// Cập nhật bước đi của User
						updateTableCells(x, y, Value.USER_VALUE);
						// Kiểm tra trạng thái của bàn cờ sau khi User đánh
						if (checkResult(Value.USER_VALUE))
						...
						caro.update(x, y, Value.USER_VALUE2);
						// Cập nhật bước đi của User
						updateTableCells(x, y, Value.USER_VALUE2);
						// Kiểm tra trạng thái của bàn cờ sau khi User đánh
						if (checkResult(Value.USER_VALUE2))
							return;
					}
				}
			}
	...
	private boolean checkResult(int player) {
		if (player == Value.USER_VALUE) {
			...
			boolean result = caro.checkWinner(Value.USER_VALUE2);
			if (result == true) {
				System.out.println("User 2 thắng!");
				JOptionPane.showMessageDialog(null, "User 2 đã thắng!");
				int currentPoint = Integer.valueOf(lblUserScore.getText()) + 1;
				lblUserScore.setText(String.valueOf(currentPoint));
				newGame();
				return true; // kết thúc màn chơi
